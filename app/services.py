...
# Importações para Push (caso precise expandir serviços futuros aqui)
# A lógica de envio principal ficou no stats_routes.py para facilitar, 
# mas é bom ter as chaves acessíveis se formos criar notificações automáticas de "Boas Vindas" aqui.
...
def inject_script_tag(store_id: str, encrypted_access_token: str):
    ...
    try:
        ...
        # O script aponta para o backend, passando o ID da loja
            ...
            if isinstance(scripts, list):
                for script in scripts:
                    if "PWA Loader" in script.get("name", ""):
                        # Se já existe, não faz nada (evita duplicidade)
...
def create_landing_page_internal(store_id: str, encrypted_access_token: str, color: str):
    ...
    try:
        ...
        # Se cor não for passada, usa preto padrão
        ...
        <div class="app-landing-wrapper">
            <div class="app-landing-card">
                <!-- Ícone (Placeholder ou Futuro Ícone Real) -->
            ...
            <script>
                ...
                function startInstall() {{
                    if (window.installPWA) {{
                        ...
                        // Fallback se o loader.js não tiver carregado ainda
                        if(isIOS) {{
        ...
        # Payload para API da Nuvemshop
        # handle: "app" tenta forçar a URL /pages/app
        payload = {
            "title": "Baixe o App", 
            "body": html_body, 
            "published": True, 
        ...
        if res.status_code == 201:
            ...
            # Se falhar, geralmente é porque a handle 'app' já existe. 
            # O sistema poderia tentar atualizar, mas por simplicidade apenas logamos.
