...
try: VAPID_CLAIMS = json.loads(raw_claims)
...
def send_webpush(subscription_info, message_body):
    try:
        ...
        webpush(subscription_info=subscription_info, data=json.dumps(message_body), vapid_private_key=VAPID_PRIVATE_KEY, vapid_claims=VAPID_CLAIMS)
        return True
    except WebPushException as ex:
        if ex.response and ex.response.status_code == 410: return "DELETE"
        return False
...
def subscribe_push(payload: PushSubscribePayload, db: Session = Depends(get_db)):
    try:
        ...
        if not endpoint or not keys.get("p256dh") or not keys.get("auth"): return {"status": "error"}

        exists = db.query(PushSubscription).filter(PushSubscription.endpoint == endpoint).first()
        if not exists:
            db.add(PushSubscription(store_id=payload.store_id, visitor_id=payload.visitor_id, endpoint=endpoint, p256dh=keys.get("p256dh"), auth=keys.get("auth"), created_at=datetime.now().isoformat()))
            db.commit()
...
def send_push_campaign(payload: PushSendPayload, store_id: str = Depends(get_current_store), db: Session = Depends(get_db)):
    ...
    # Histórico
    db.add(PushHistory(store_id=store_id, title=payload.title, message=payload.message, url=payload.url, sent_count=sent_count, created_at=datetime.now().isoformat()))
    db.commit()
...
def get_push_history(store_id: str = Depends(get_current_store), db: Session = Depends(get_db)):
    return db.query(PushHistory).filter(PushHistory.store_id == store_id).order_by(PushHistory.id.desc()).all()
